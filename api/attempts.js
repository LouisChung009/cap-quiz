...
export default async function handler(request, response) {
  ...
  try {
    ...
    const sql = database(); await sql`INSERT INTO question_attempts (user_id, question_id, subject, unit_name, knowledge_point, is_correct, answered_at) VALUES (${claims.sub}, ${questionId}, ${subject}, ${String(unit || "").slice(0, 80)}, ${String(knowledgePoint || "").slice(0, 80)}, ${correct}, ${new Date(answeredAt || Date.now()).toISOString()})`;
